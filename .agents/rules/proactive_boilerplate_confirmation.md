...
8. **Banner Ad Placement:** If the app uses a banner ad (`AdBannerWidget`), it MUST always be placed at the absolute bottom of the screen (e.g., using `bottomNavigationBar` or `Align(alignment: Alignment.bottomCenter)`) to ensure a consistent user experience and prevent interference with core app functionality.
9. **App Versioning Rules:** When updating the app version in `pubspec.yaml`, the AI MUST use the `version: x.y.z+n` format and ALWAYS increment the `+n` (versionCode) by at least 1 compared to the previous release. Never omit the `+n` part, as it is strictly required by Google Play Console for updates.
